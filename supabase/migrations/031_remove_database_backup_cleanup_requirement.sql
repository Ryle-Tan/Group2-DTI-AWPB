-- Remove the database-backup confirmation step from archive cleanup.
-- The app only creates CSV archives, so cleanup should require the CSV backup
-- record and not a separate SQL/database backup confirmation.

DROP FUNCTION IF EXISTS public.admin_confirm_database_backup(INTEGER, TEXT);
...
ALTER TABLE public.archive_backups
    DROP COLUMN IF EXISTS sql_backup_filename,
    DROP COLUMN IF EXISTS sql_backup_marked_by,
    DROP COLUMN IF EXISTS sql_backup_marked_by_name,
    DROP COLUMN IF EXISTS sql_backup_marked_at;
