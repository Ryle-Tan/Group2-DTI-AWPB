-- Allow archive cleanup to be retried when a year was marked complete but
-- matching entries or budget transactions still remain.
...
BEGIN
    ...
    IF v_backup.cleanup_completed_at IS NOT NULL
        AND v_entry_count = 0
        AND v_deleted_budget_transactions = 0
    THEN
